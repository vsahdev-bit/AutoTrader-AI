...
export default function Dashboard() {
  ...
  const [connectorIssues, setConnectorIssues] = useState(0)
  ...
  // Load connector summary (used to show issue indicator in Quick Actions)
  useEffect(() => {
    let cancelled = false

    const fetchConnectorSummary = async () => {
      try {
        const resp = await fetch('/api/v1/connectors/summary')
        if (!resp.ok) return
        const data = await resp.json()
        const issues = (data?.error ?? 0) + (data?.disconnected ?? 0)
        if (!cancelled) setConnectorIssues(issues)
      } catch {
        // ignore
      }
    }
    ...
    const interval = setInterval(fetchConnectorSummary, 5 * 60 * 1000)
                ...
                >
                  ...
                  {connectorIssues > 0 && (
                    <span
                      ...
                      title={`${connectorIssues} connector${connectorIssues > 1 ? 's' : ''} with issues`}
                    >
